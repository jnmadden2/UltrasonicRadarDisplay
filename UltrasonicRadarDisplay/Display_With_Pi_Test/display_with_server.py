...
READ_TYPE = 2  # 0 for file, 1 for serial, 2 for Raspberry pi
...
ip_address ="10.29.183.189"  # Replace with the RPI IP, this will be 10.xxx.xxx.xxx
...
is_reading = False  # Global flag to indicate if read_from_serial or reading from wifi is running
...
def mouse_moved(evt):
    ...
    if plot.sceneBoundingRect().contains(pos):
        ...
        if angle < 0:
            angle += 360
        if 0 <= angle <= 180 and distance <= (1000 + 10) and distance >= (
        -10):  # Only display within the semi-circular area
...
def process_data():
    global points, aged_points, point_index
    if 1: #mystr:
        # Add your data processing logic here
        print("Processing data:", mystr)
        # Example: Parse the data and add points
        data_points = mystr.strip().split('.')
        for point in data_points:
            if ',' in point:
                angle, distance = map(float, point.split(','))
                points.append((angle, distance))
                for p in aged_points:
                    p['age'] += 1
                display_next_point()
...
# Timer function to repeatedly check the server for data
def check_server_for_data():
    global is_reading
    if is_reading:
        return  # Exit if the function is already running
    is_reading = True
...
if READ_TYPE == 2:
    ...
    # pass
    timer = QtCore.QTimer()
    timer.timeout.connect(check_server_for_data)
    timer.start(200)  # Check the server every 200 mili seconds
